...
def publish(publisher, topic, events):
   numobs = len(events)
   if numobs > 0:
       logging.info('Publishing {0} events from {1}'.format(numobs, get_timestamp(>
...
def simulate(topic, ifp, firstObsTime, programStart, speedFactor):
   ...
   def compute_sleep_secs(obs_time):
        ...
        sim_time_elapsed = ((obs_time - firstObsTime).days * 86400.0 + (obs_time ->
...
if __name__ == '__main__':
   ...
   parser.add_argument('--speedFactor', help='Example: 60 implies 1 hour of data s>
   parser.add_argument('--project', help='Example: --project $DEVSHELL_PROJECT_ID'>
   ...
   try:
     ...
     # For get_topic, create a GetTopicRequest object
     get_topic_request = types.GetTopicRequest(topic=event_type)
     publisher.get_topic(request=get_topic_request)
     print('Topic exists')

   except:
     # publisher.create_topic(event_type)
     # logging.info('Creating pub/sub topic {}'.format(TOPIC))
     # If the topic does not exist (or get_topic fails for other reasons), create >
     # For create_topic, create a Topic object
     # The 'name' field of the Topic object should be the full topic path
     create_topic_request = types.Topic(name=event_type)
     publisher.create_topic(request=create_topic_request)
     print('Topic created')
